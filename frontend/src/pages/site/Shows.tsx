...
type ShowItem = { dia: string; mesAbrev: string; evento: string; cidade: string; dataBR: string };
...
export default function Shows() {
    ...
    EventoApi.listar()
      .then((eventos) => {
        ...
        for (const e of futuros) {
          ...
          const item: ShowItem = { dia: String(d.getDate()).padStart(2, '0'), mesAbrev: ABREV[d.getMonth()], evento: e.nomeEvento, cidade: `${e.cidade} — ${e.uf}`, dataBR: d.toLocaleDateString('pt-BR') };
  ...
  const linkDetalhes = (s: ShowItem) => {
    if (!numeroFinal) return '/#contato';
    const msg = `Quero saber mais sobre o show "${s.evento}" no dia "${s.dataBR}"`;
